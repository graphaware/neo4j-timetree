...
into the `plugins` directory. After Neo4j restart, you will be able to use the REST APIs of the TimeTree.
...
In order to be able to ask interesting queries, such as "show me all emails sent in a specific month", people often built
...
For example, you can ask the library "give me a node representing 24th May 2014" and you'll get it and can start linking to it.
...
* http://your-server-address:7474/graphaware/timetree/{time} to get a node representing a time instant, where time must be replaced by a long number representing the number of milliseconds from 1/1/1970. The default resolution is Day and the default time zone is UTC
...
of the node representing the hour.
